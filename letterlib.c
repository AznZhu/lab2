...
{
	// loop until a null character in the array is hit	for(int i = 0;word[i] != '\0';i++)
	{	
...
// Draws if word[i] matches a case
// Draws "missing letter" otherwise
		switch(letter){
			case '2':
		...
		//enter, tab, space, and null exit switch	
			case '\n': 
		    case '\t':
		    case '\0':
		    case ' ':
...
void drawCharQuestion(double ulCornerX, double ulCornerY, double height)
{
    ...
    // Calculate start and end coordinates and draw first line
...
void drawCharY(double ulCornerX, double ulCornerY, double side)
{
    ...
    // calculate coordinates of all corners
    double height = side;
...
{
    // rectangle width is .75*length
    // length represents height. Program uses height to scale the graphic, and upper left coords for starting point.
    double width = .75 * height;    
    ...
    //Draws line segments to create outer outline
    ...
    //Draws line segment to create inner outline
...
void drawCharR(double ulCornerX, double ulCornerY, double height)
{
    ...
    double PI = 3.14159;
    double width = .75 * height;
	double radiusLg = ((.8*width)/2;
	double radiusSm = (width - (.6*width))/2;
	double circCenterX = ulCornerX+width-radiusLg;
	double circCenterY = ulCornerY + radiusLg;
	double theta;
	double thetaNext;
	double thetaIncrement = 2.0 / (2 * PI * radiusLg); //radians
...
gfx_line(0.1*xx+ulCornerX,0.1*yy+ulCornerY,1*xx+ulCornerX,0.1*yy+ulCornerY); //Top line of S of $
...
gfx_line(1*xx+ulCornerX,.275*yy+ulCornerY,0.3*xx+ulCornerX,.275*yy+ulCornerY); //Right to left line of S
gfx_line(0.3*xx+ulCornerX,.275*yy+ulCornerY,0.3*xx+ulCornerX,.40*yy+ulCornerY); //Left small segment of S
gfx_line(0.3*xx+ulCornerX,.40*yy+ulCornerY,1*xx+ulCornerX,.40*yy+ulCornerY); //Left to right line of S
...
gfx_line(.40*xx+ulCornerX,1*yy+ulCornerY,.70*xx+ulCornerX,1*yy+ulCornerY); //Bottom of center Bar of $
}
